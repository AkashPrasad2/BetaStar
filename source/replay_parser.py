...
from obs_spec import (
    ...
    IDX_SUPPLY_USED, IDX_SUPPLY_CAP, SUPPLY_NORM, ACTION_ID,
...
def _action_legal_numpy(obs: list[float], action_id: int) -> tuple[bool, str]:
    ...
    rules = {
        "train_probe": (has_nexus, "needs nexus"),
        "build_pylon": (True, ""),
        ...
        "train_stalker": (poc_gateway_type and poc_cybcore, "needs gateway/warpgate and poc_cybcore"),
        "train_immortal": (poc_robo, "needs poc_robo"),
        "train_voidray": (poc_stargate, "needs poc_stargate"),
        "train_carrier": (poc_stargate and has_fleet, "needs poc_stargate and has_fleet"),
        "warp_in_zealot": (poc_warpgate, "needs warpgate"),
        "warp_in_stalker": (poc_warpgate and poc_cybcore, "needs warpgate and poc_cybcore"),
    ...
    ok, reason = rules.get(ACTION_NAMES[action_id]
                           if 0 <= action_id < len(ACTION_NAMES)
                           else None, (False, "unknown action"))
    if not ok:
        return ok, reason
...
class ReplayParser:
    def __init__(
        ...
        self.EVENT_TO_ACTION = {
            "TrainProbe": ACTION_ID["train_probe"],
            "BuildPylon": ACTION_ID["build_pylon"],
            "BuildGateway": ACTION_ID["build_gateway"],
            "BuildCyberneticsCore": ACTION_ID["build_cyberneticscore"],
            "BuildAssimilator": ACTION_ID["build_assimilator"],
            "BuildNexus": ACTION_ID["build_nexus"],
            "BuildForge": ACTION_ID["build_forge"],
            "BuildStargate": ACTION_ID["build_stargate"],
            "BuildRoboticsFacility": ACTION_ID["build_robotics_facility"],
            "BuildTwilightCouncil": ACTION_ID["build_twilight_council"],
            "BuildPhotonCannon": ACTION_ID["build_photon_cannon"],
            "BuildFleetBeacon": ACTION_ID["build_fleet_beacon"],
            "BuildTemplarArchive": ACTION_ID["build_templar_archive"],
            "BuildRoboticsBay": ACTION_ID["build_robotics_bay"],
            "BuildShieldBattery": ACTION_ID["build_shield_battery"],
            "TrainZealot": ACTION_ID["train_zealot"],
            "TrainStalker": ACTION_ID["train_stalker"],
            "TrainImmortal": ACTION_ID["train_immortal"],
            "TrainVoidRay": ACTION_ID["train_voidray"],
            "TrainCarrier": ACTION_ID["train_carrier"],
            # "TrainHighTemplar" removed: train_high_templar had zero labels
            "WarpInZealot": ACTION_ID["warp_in_zealot"],
            "WarpInStalker": ACTION_ID["warp_in_stalker"],
            "WarpInHighTemplar": ACTION_ID["warp_in_high_templar"],
            "ResearchCharge": ACTION_ID["research_charge"],
            "ResearchWarpGate": ACTION_ID["research_warp_gate"],
            "UpgradeGroundWeapons1": ACTION_ID["upgrade_ground_weapons"],
            "UpgradeGroundWeapons2": ACTION_ID["upgrade_ground_weapons"],
            "UpgradeGroundWeapons3": ACTION_ID["upgrade_ground_weapons"],
            "UpgradeAirWeapons1": ACTION_ID["upgrade_air_weapons"],
            "UpgradeAirWeapons2": ACTION_ID["upgrade_air_weapons"],
            ...
            "TrainColossus": ACTION_ID["train_colossus"],
            # "WarpInAdept" removed: warp_in_adept had zero labels
        }
